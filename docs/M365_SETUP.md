...
- La Function App consulta el buzón restringido mediante Graph, procesa cada adjunto PDF y archiva los resultados en SharePoint.
- Document Intelligence clasifica y extrae los datos de los documentos fiscales.
- La Function App usa su identidad administrada con `Sites.Selected` para consultar `ExtractosBancarios` cada 10 minutos, leer la primera hoja y guardar lotes y movimientos en SharePoint Lists.
- El núcleo TypeScript conserva validación, idempotencia, nomenclatura, importación, conciliación y resolución auditable de excepciones.
...
El script crea también `MaestroProveedores`, `ProcesosFacturas` y `RegistroFacturas`, y añade de forma no destructiva las columnas nuevas a listas ya existentes. Antes de probar la aceptación automática, añade una fila por proveedor con `CodigoProveedor`, `RazonSocial`, `NIF`, `Aliases` (uno por línea o separados por `;`) y `Activo = Sí`. El NIF debe ser único entre proveedores activos. Si Document Intelligence entrega NIF, la Function exige una coincidencia exacta; solo cuando el NIF no está disponible compara la razón social y los alias normalizados. Una ausencia, contradicción o coincidencia múltiple obliga a revisión.

`AceptaConfianzaReducida` se deja vacío o en `No` por defecto. Solo se habilita tras revisar una familia estable de facturas de un proveedor. Con `Sí`, la Function permite el perfil acotado únicamente cuando el NIF extraído coincide de forma exacta y única con ese proveedor activo; conserva todos los campos obligatorios, formato y la igualdad base + IVA = total. Actualmente el perfil exige como mínimo NIF `0,68`, IVA `0,68` y total `0,88`; los demás umbrales siguen siendo los generales.
...
No se guardan secretos en el repositorio ni en `.env`. La Function obtiene tokens de Graph y Document Intelligence mediante identidad administrada.
...
1. Confirmar acceso a los centros de administración de Microsoft 365 y Microsoft Entra.
...
5. Desplegar y configurar Document Intelligence.
6. Crear la carpeta bancaria restringida y las listas de lotes y movimientos.
7. Ejecutar una prueba con facturas y extractos anonimizados.
