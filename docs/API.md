...
Los estados admitidos son `Pendiente`, `Programado`, `Parcial` y `Cancelado`; el endpoint rechaza de forma explícita `Pagado` o `Pagada`. El pago solo puede confirmarse mediante conciliación bancaria y una decisión humana. El lote conserva el nombre y una huella canónica de las filas de origen, por lo que una reserialización técnica de SharePoint/Office no crea otro lote. La clave idempotente combina el identificador de previsión, factura, proveedor, fecha, importe previsto y moneda. Los duplicados conocidos se informan sin bloquear el lote; reutilizar un `PrevisionId` existente también se considera duplicado para evitar crear una segunda previsión con datos alterados.
