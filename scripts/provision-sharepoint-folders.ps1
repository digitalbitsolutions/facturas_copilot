...
if ($UseAzureCliToken) {
    ...
    if (-not $azCommand -and (Test-Path "$env:ProgramFiles\Microsoft SDKs\Azure\CLI2\wbin\az.cmd")) { $azCommand = Get-Item "$env:ProgramFiles\Microsoft SDKs\Azure\CLI2\wbin\az.cmd" }
...
$created = @()
$alreadyPresent = @()
foreach ($folderName in @('Facturas', 'ExtractosBancarios', 'PrevisionesPagos', 'Configuracion', 'Procesados', 'Errores', 'ProcesadosPrevisiones', 'ErroresPrevisiones')) {
