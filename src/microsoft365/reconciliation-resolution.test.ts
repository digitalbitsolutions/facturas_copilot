...
test("persists a review-only proposal and marks its movement for review", async () => {
  ...
  const client = graph((path, init) => {
    ...
    if (path.includes("recs/items?%24expand")) return Response.json({ value: [] });
    if (path.includes("moves/items?%24expand")) return Response.json({ value: [{ id: "movement-item", fields: { MovimientoId: "mov-1" } }] });
