...
test("persists a review-only proposal and marks its movement for review", async () => {
  ...
  const client = graph((path, init) => {
    ...
    if (path.includes("recs/items?expand")) return Response.json({ value: [] });
  ...
  assert.equal((calls.find((call) => call.path.includes("recs/items?expand"))?.headers as Record<string, string> | undefined)?.Prefer, "HonorNonIndexedQueriesWarningMayFailRandomly");
